...
#include "../linux-3.10.44/include/generated/autoconf.h"
...
#endif
//proc ops struct
static const struct file_operations procGmac_fops = {
        .read = (read_proc_t*)&RegReadMain,
#if defined (CONFIG_ETHTOOL) && defined (CONFIG_RAETH_ROUTER)
        .write = (write_proc_t*)&change_phyid,
#endif
};
static const struct file_operations procSkbFree_fops = {
        .read = (read_proc_t*)&SkbFreeRead,
};
static const struct file_operations procTxRing_fops = {
        .read = (read_proc_t*)&TxRingRead,
};
static const struct file_operations procRxRing_fops = {
        .read = (read_proc_t*)&RxRingRead,
};
static const struct file_operations procSysCP0_fops = {
        .read = (read_proc_t*)&CP0RegRead,
};
static const struct file_operations procSkbFree_fops = {
        .read = (read_proc_t*)&SkbFreeRead,
};
...
int debug_proc_init(void)
{
    ...
    procGmac = proc_create(PROCREG_GMAC, 0, procRegDir, &procGmac_fops)
    procSkbFree = proc_create(PROCREG_SKBFREE, 0, procRegDir, &procSkbFree_fops)
    procTxRing = proc_create(PROCREG_TXRING, 0, procRegDir, &procTxRing_fops)
    procRxRing = proc_create(PROCREG_RXRING, 0, procRegDir, &procRxRing_fops)
    procSysCP0 = proc_create(PROCREG_CP0, 0, procRegDir, &procSysCP0_fops)
   
#if defined(CONFIG_RAETH_TSO)
    if ((procNumOfTxd = proc_create(PROCREG_NUM_OF_TXD, 0, procRegDir)))
	 procNumOfTxd->read_proc = (read_proc_t*)&NumOfTxdRead;
	 procNumOfTxd->write_proc = (write_proc_t*)&NumOfTxdWrite;
    
    if ((procTsoLen = proc_create(PROCREG_TSO_LEN, 0, procRegDir)))
	 procTsoLen->read_proc = (read_proc_t*)&TsoLenRead;
	 procTsoLen->write_proc = (write_proc_t*)&TsoLenWrite;
#endif

#if defined(CONFIG_RAETH_LRO)
    if ((procLroStats = proc_create(PROCREG_LRO_STATS, 0, procRegDir)))
	 procLroStats->read_proc = (read_proc_t*)&LroStatsRead;
	 procLroStats->write_proc = (write_proc_t*)&LroStatsWrite;
#endif

#if defined(CONFIG_RAETH_QOS)
    if ((procRaQOS = proc_create(PROCREG_RAQOS, 0, procRegDir)))
	 procRaQOS->read_proc = (read_proc_t*)&RaQOSRegRead;
#endif

#if defined(CONFIG_USER_SNMPD)
    procRaSnmp = proc_create(PROCREG_SNMP, S_IRUGO, procRegDir);
    if (procRaSnmp == NULL)
    ...
    if ((procEswCnt = proc_create( PROCREG_ESW_CNT, 0, procRegDir))){
	 procEswCnt->read_proc = (read_proc_t*)&EswCntRead;
    }

#if defined (TASKLET_WORKQUEUE_SW)
    if ((procSCHE = proc_create(PROCREG_SCHE, 0, procRegDir))){
