...
static inline double rt2860v2_freq2float(const struct iw_freq *in)
{
	int		i;
...
static inline int rt2860v2_extract_event(struct stream_descr *stream, struct iw_event *iwe, int wev)
{
...
static inline void rt2860v2_fill_wpa(unsigned char *iebuf, int ielen, struct iwinfo_scanlist_entry *e)
{
...
static inline void rt2860v2_fill_entry(struct stream_descr *stream, struct iw_event *event,
	struct iw_range *iw_range, int has_range, struct iwinfo_scanlist_entry *e)
{
	...
	switch(event->cmd)
	{
		...
		case SIOCGIWFREQ:
			...
			{
				freq = rt2860v2_freq2float(&(event->u.freq));

				for(i = 0; i < iw_range->num_frequency; i++)
				{
					if( rt2860v2_freq2float(&iw_range->freq[i]) == freq )
					{
...
#endif
		 case IWEVGENIE:
			rt2860v2_fill_wpa(event->u.data.pointer, event->u.data.length, e);
			break;
	}
}
...
int rt2860v2_get_scanlist(const char *ifname, char *buf, int *len)
{
	...
	if( rt2860v2_ioctl(ifname, SIOCGIWRANGE, &wrq) >= 0 )
	{
		...
		/* Initiate Scanning */
		if( rt2860v2_ioctl(ifname, SIOCSIWSCAN, &wrq) >= 0 )
		{
			timeout -= tv.tv_usec;
		...
		realloc:
					...
					if( rt2860v2_ioctl(ifname, SIOCGIWSCAN, &wrq) )
					{
			...
			if( wrq.u.data.length )
			{
				...
				do
				{
					...
					ret = rt2860v2_extract_event(&stream, &iwe, range.we_version_compiled);
					...
					if(ret >= 0)
					{
						...
						rt2860v2_fill_entry(&stream, &iwe, &range, has_range, &e);
					}
